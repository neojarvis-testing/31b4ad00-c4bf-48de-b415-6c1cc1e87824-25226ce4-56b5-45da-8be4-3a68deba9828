...
const validationSchema = Yup.object({
  ...
  gracePreriodMonths: Yup.number().min(0).required("Grace period is required"),
...
const LoanFormPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchLoan = async () => {
      try {
        const response = await getLoanById(id, token);
  ...
  const handleSubmit = async (values) => {
    try {
      if (id) {
        await updateLoan(id, values, token);
      } else {
  ...
  return (
    <>
      ...
      <div style={{ padding: "20px", maxWidth: "700px", margin: "auto" }}>
        <h2>{id ? "Edit Loan Product" : "Add Loan Product"}</h2>
        <Formik
